...
// Global. - namespace WPGraphQL\BlockEditor
	...
	private function setup_constants() {
		...
		// Plugin version.
		if ( ! defined( 'WPGRAPHQL_BLOCK_EDITOR_VERSION' ) ) {
			define( 'WPGRAPHQL_BLOCK_EDITOR_VERSION', '0.0.1' );
		}

		// Plugin Folder Path.
		if ( ! defined( 'WPGRAPHQL_BLOCK_EDITOR_PLUGIN_DIR' ) ) {
			define( 'WPGRAPHQL_BLOCK_EDITOR_PLUGIN_DIR', plugin_dir_path( $main_file_path ) );
		}

		// Plugin Root File.
		if ( ! defined( 'WPGRAPHQL_BLOCK_EDITOR_PLUGIN_FILE' ) ) {
			define( 'WPGRAPHQL_BLOCK_EDITOR_PLUGIN_FILE', $main_file_path );
		}

		// Whether to autoload the files or not.
		if ( ! defined( 'WPGRAPHQL_BLOCK_EDITOR_AUTOLOAD' ) ) {
			define( 'WPGRAPHQL_BLOCK_EDITOR_AUTOLOAD', true );
		}

		// The minimum version of PHP this plugin requires to work properly
		if ( ! defined( 'WPGRAPHQL_BLOCK_EDITOR_MIN_PHP_VERSION' ) ) {
			define( 'WPGRAPHQL_BLOCK_EDITOR_MIN_PHP_VERSION', '7.1' );
		}
	...
	private function includes() {
		...
		/**
		 * WPGRAPHQL_BLOCK_EDITOR_AUTOLOAD can be set to "false" to prevent the autoloader from running.
		 ...
		 */
		if ( defined( 'WPGRAPHQL_BLOCK_EDITOR_AUTOLOAD' ) && true === WPGRAPHQL_BLOCK_EDITOR_AUTOLOAD ) {
				...
				add_action(
					...
					function () {
						...
						echo sprintf(
							...
							'<p>%s</p>' .
							'</div>',
							__( 'WPGraphQL Block Editor will not work without WPGraphQL installed and active.', 'wp-graphql' )
						);
					}
				);
